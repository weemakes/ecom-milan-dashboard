  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({
        ...
        sku: sku.trim() || null,
        occasion: product ? (product.occasion || null) : null,
        landing_section: product ? (product.landing_section || null) : null,
        featured_type: product ? (product.featured_type || null) : null,
        images,
        variants,
        is_active: isActive,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex items-center justify-between p-3.5 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40">
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${isFeatured ? 'bg-indigo-600' : 'bg-zinc-300 dark:bg-zinc-700'
              }`}
          >
            <span
